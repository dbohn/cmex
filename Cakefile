...
task 'build', 'Builds coffeescript sources', ->
    exec 'coffee --compile --output public/assets/js/admin/ app/assets/javascripts/', (err, stdout, stderr) ->
        throw err if err
...
task 'build:watch', 'Watches for coffeescript changes', ->
    ...
    exec 'coffee --compile --watch --output public/assets/js/admin/ app/assets/javascripts', (err, stdout, stderr) ->
        throw err if err
        console.log stdout + stderr